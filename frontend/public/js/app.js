...
function showSection(id) {
  ...
  const renderers = {
    ...
    market:    renderMarket,
    risk:      renderRisk,
    liveMap:   function(mc) { if (typeof destroyLiveMap === 'function') destroyLiveMap(); renderLiveMap(mc); },
  };
...
async function initTicker() {
  const items = MOCK_DATA.ticker;
  const html  = [...items, ...items].map(function(i) {
...
async function refreshVesselCount() {
  const data = await API.Vessels.count().catch(function() { return null; });
  const total = (data && data.total) ? data.total : MOCK_DATA.vesselCount.total;
...
function showAlertModal() {
  ...
  if (!modal) return;
  const risks = MOCK_DATA.risks.filter(function(r) { return r.level !== 'LOW'; });
  document.getElementById('modal-alerts').innerHTML = risks.map(function(r) {
...
window.addEventListener('DOMContentLoaded', async function() {
  ...
  checkApiHealth();
  setInterval(refreshVesselCount, 10000);
});
